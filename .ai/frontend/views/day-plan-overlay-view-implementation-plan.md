...
Routowalny dialog/drawer na trasie `/?day=YYYY-MM-DD` pozwala wybrać lub zmienić danie dla wskazanego dnia. Otwierany z tygodniowego widoku listy dni i po zamknięciu wraca do tego samego tygodnia (`weekIndex` bez zmian). Dzisiejszy dzień może być dodatkowo oznaczony w nagłówku. Korzysta z `/api/dishes` (sort `usage_prio` + filtry tagów), `/api/tags` oraz `/api/day-plans/{day}` (PUT) z walidacją i obsługą pustej bazy dań.
...
6. Zintegrować z tygodniową nawigacją listy dni: po zamknięciu wracamy do tego samego `weekIndex`; na mobile blokada swipe tygodnia w trakcie otwartej nakładki.
